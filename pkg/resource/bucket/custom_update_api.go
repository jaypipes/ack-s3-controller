...
func (rm *resourceManager) customUpdateBucket(
	...
	updated = desired
	if aclChanged(desired, latest) {
		updated, err = rm.updateACL(ctx, updated)
		if err != nil {
			return nil, err
		}
	}
	if objectLockChanged(desired, latest) {
...
// aclChanged returns true if the ACL configuration of the supplied desired and
// latest Bucket resources is different
func aclChanged(
	desired *resource,
	latest *resource,
) bool {
	...
	if dspec.ACL == nil && lspec.ACL != nil {
		return true
	}
	if lspec.ACL == nil {
		return true
	}
	if *dspec.ACL != *lspec.ACL {
		return true
	}

	if dspec.GrantFullControl == nil && lspec.GrantFullControl != nil {
		return true
	}
	if lspec.GrantFullControl == nil {
		return true
	}
	if *dspec.GrantFullControl != *lspec.GrantFullControl {
		return true
	}

	if dspec.GrantRead == nil && lspec.GrantRead != nil {
		return true
	}
	if lspec.GrantRead == nil {
		return true
	}
	if *dspec.GrantRead != *lspec.GrantRead {
		return true
	}

	if dspec.GrantReadACP == nil && lspec.GrantReadACP != nil {
		return true
	}
	if lspec.GrantReadACP == nil {
		return true
	}
	if *dspec.GrantReadACP != *lspec.GrantReadACP {
		return true
	}

	if dspec.GrantWrite == nil && lspec.GrantWrite != nil {
		return true
	}
	if lspec.GrantWrite == nil {
		return true
	}
	if *dspec.GrantWrite != *lspec.GrantWrite {
		return true
	}

	if dspec.GrantWriteACP == nil && lspec.GrantWriteACP != nil {
		return true
	}
	if lspec.GrantWriteACP == nil {
		return true
	}
	if *dspec.GrantWriteACP != *lspec.GrantWriteACP {
		return true
	}
	return false
}
...
// objectLockChanged returns true if the object lock configuration of the
// supplied desired and latest Bucket resources is different
func objectLockChanged(
	desired *resource,
	latest *resource,
) bool {
	dspec := desired.ko.Spec
	...
	if dspec.ObjectLockEnabledForBucket == nil && lspec.ObjectLockEnabledForBucket != nil {
		return true
	}
	if lspec.ObjectLockEnabledForBucket == nil {
		return true
	}
	return *dspec.ObjectLockEnabledForBucket != *lspec.ObjectLockEnabledForBucket
}
